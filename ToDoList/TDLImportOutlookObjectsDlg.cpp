...
CTDLImportOutlookObjectsDlg::CTDLImportOutlookObjectsDlg(OutlookAPI::_MailItem& refItem, CWnd* pParent /*=NULL*/)
	: CDialog(CTDLImportOutlookObjectsDlg::IDD, pParent), 
	m_lcFieldMapping(TRUE, FALSE), m_refItem(refItem), m_bHideUnmapped(FALSE), m_bHideConfidential(TRUE)
{
	BuildMasterMapping();
}
...
BEGIN_MESSAGE_MAP(CTDLImportOutlookObjectsDlg, CDialog)
...
void CTDLImportOutlookObjectsDlg::RemoveUnwantedAttributes(CTDCCsvColumnMapping& aMapping) const
{
	if (m_bHideUnmapped || m_bHideConfidential)
	{
		...
		while (nField--)
		{
			...
			else if (m_bHideConfidential && COutlookHelper::IsConfidential((OUTLOOK_FIELDTYPE)col.dwItemData))
			{
				aMapping.RemoveAt(nField);
			}
		}
...
void CTDLImportOutlookObjectsDlg::OnHideAttributes() 
{
	...
	{
		COutlookHelper::ResetDenyConfidential();
		COutlookHelper::GetItemData(m_refItem, m_mapRefData, TRUE);
