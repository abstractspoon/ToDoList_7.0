...
BOOL CTDLImportExportMgr::ImportTaskList(LPCTSTR szSrcFile, ITaskList* pDestTasks, int nByImporter, BOOL bSilent) const
{
	...
	if (CImportExportMgr::ImportTaskList(szSrcFile, pDestTasks, nByImporter, bSilent, &prefs))
	{
...
void CTDLImportExportMgr::SetTaskAttributesToDefaults(ITaskList* pTasks, HTASKITEM hTask, BOOL bAndSiblings) const
{
	...
	if (!pTasks9->TaskHasAttribute(hTask, TDL_TASKTIMEESTIMATE))
		pTasks9->SetTaskTimeEstimate(hTask, m_tdiDefault.dTimeEstimate, (TCHAR)m_tdiDefault.nTimeEstUnits);
	
	if (!pTasks9->TaskHasAttribute(hTask, TDL_TASKTIMESPENT))
		pTasks9->SetTaskTimeSpent(hTask, m_tdiDefault.dTimeSpent, (TCHAR)m_tdiDefault.nTimeSpentUnits);
