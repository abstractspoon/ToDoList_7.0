...
CPreferencesUITasklistColorsPage::CPreferencesUITasklistColorsPage() : 
	...
	m_nTextColorOption(COLOROPT_DEFAULT), m_cbAttributes(ACBS_ALLOWDELETE), m_nColorAttribute(TDCA_NONE)
...
BEGIN_MESSAGE_MAP(CPreferencesUITasklistColorsPage, CPreferencesPageBase)
	...
	ON_REGISTERED_MESSAGE(WM_ACBN_ITEMADDED, OnAttribAdded)
	ON_REGISTERED_MESSAGE(WM_ACBN_ITEMDELETED, OnAttribDeleted)
END_MESSAGE_MAP()
	...
	if (m_bColorPriority)
	{
		...
		else
		{
			...
			else // RGB Gradient
			{
				...
				{
					double dRed = (redLow * (10 - nPriority) / 10.0) + (redHigh * nPriority / 10.0);
					double dGreen = (greenLow * (10 - nPriority) / 10.0) + (greenHigh * nPriority / 10.0);
					double dBlue = (blueLow * (10 - nPriority) / 10.0) + (blueHigh * nPriority / 10.0);
...
LRESULT CPreferencesUITasklistColorsPage::OnAttribAdded(WPARAM /*wParam*/, LPARAM lParam)
{
   ...
   if (!sAttrib.IsEmpty() && FindAttribColor(sAttrib) == -1)
   {
		...
		// notify parent
		GetParent()->SendMessage(WM_PUITCP_ATTRIBCHANGE, MAKEWPARAM(1, m_nColorAttribute), lParam);
...
LRESULT CPreferencesUITasklistColorsPage::OnAttribDeleted(WPARAM /*wParam*/, LPARAM lParam)
{
   ...
   if (!sAttrib.IsEmpty())
   {
	   ...
	   // notify parent
	   GetParent()->SendMessage(WM_PUITCP_ATTRIBCHANGE, MAKEWPARAM(0, m_nColorAttribute), lParam);
...
void CPreferencesUITasklistColorsPage::AddAttributes(TDC_ATTRIBUTE nAttrib, const CStringArray& aAttrib)
{
	...
	for (int nItem = 0; nItem < aAttrib.GetSize(); nItem++)
	{
		ac.sAttrib = aAttrib[nItem];
		ac.color = 0;
		
		if (FindAttribColor(ac.sAttrib) == -1)
		{
			m_aAttribColors.Add(ac);
...
void CPreferencesUITasklistColorsPage::AddDefaultListItemsToAttributeColors(const CPreferences& prefs)
{
	...
	{
	case TDCA_CATEGORY:
		nDefColor = prefs.GetProfileArray(_T("Preferences\\CategoryList"), aDefAttribs);
		break;

	case TDCA_ALLOCBY:
		nDefColor = prefs.GetProfileArray(_T("Preferences\\AllocByList"), aDefAttribs);
		break;

	case TDCA_ALLOCTO:
		nDefColor = prefs.GetProfileArray(_T("Preferences\\AllocToList"), aDefAttribs);
		break;

	case TDCA_STATUS:
		nDefColor = prefs.GetProfileArray(_T("Preferences\\StatusList"), aDefAttribs);
		break;
	}
