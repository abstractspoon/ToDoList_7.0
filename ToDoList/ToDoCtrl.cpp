...
//////////////////////////////////////////////////////////////////////////////
// private method to avoid including StdioFileEx in header
...
const UINT WM_TDC_FIXUPPOSTDROPSELECTION		= (WM_APP + 1);
const UINT WM_TDC_REFRESHPERCENTSPINVISIBILITY	= (WM_APP + 2);
const UINT WM_TDC_RECREATERECURRINGTASK			= (WM_APP + 3);
...
BOOL CToDoCtrl::ParseTaskLink(const CString& sLink, DWORD& dwTaskID, CString& sFile)
{
	return CTDCTaskCtrlBase::ParseTaskLink(sLink, dwTaskID, sFile);
}

BOOL CToDoCtrl::IsReservedShortcut(DWORD dwShortcut)
{
	return CTDCTaskCtrlBase::IsReservedShortcut(dwShortcut);

}
void CToDoCtrl::EnableExtendedSelection(BOOL bCtrl, BOOL bShift)
{
	CTDCTaskCtrlBase::EnableExtendedSelection(bCtrl, bShift);
}
...
BOOL CToDoCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, BOOL bVisible, BOOL bEnabled)
{
	...
	return CRuntimeDlg::Create(NULL, dwStyle, WS_EX_CONTROLPARENT, rect, pParentWnd, nID);
}
...
void CToDoCtrl::SetMaximizeState(TDC_MAXSTATE nState)
{
	...
	if (m_nMaxState != nState)
	{
		...
		if (GetSafeHwnd())
		{
			InvalidateRect(GetSplitterRect());
			Resize();
...
void CToDoCtrl::ReposComments(CDeferWndMove* pDWM, CRect& rAvailable /*in/out*/) 
{
	...
	{
		BOOL bStackCommentsAndControls = (GetStackCommentsAndControls() && GetVisibleEditFields().GetSize());
...
int CToDoCtrl::GetAutoListData(TDCAUTOLISTDATA& tld, BOOL bIncDefault) const
{
...
void CToDoCtrl::ShowTaskHasIncompleteDependenciesError(const CString& sIncomplete)
{
	...
	{
		int nRet = AfxMessageBox(CEnString(IDS_TDC_SELTASKHASDEPENDENCY), MB_YESNOCANCEL | MB_ICONEXCLAMATION);
...
{
	CToDoCtrlData::SetInheritedParentAttributes(aAttribs, bUpdateAttrib);
}
	...
	if (bDueDate) // dtNext is the new due date
	{
		...
		else // bump dates by required amount
		{
			if (bHasDue)
			{
				...
				m_data.OffsetTaskDate(dwNewTaskID, TDCD_DUEDATE, nOffsetDays, TDCO_DAYS, TRUE, FALSE);
			}
			else
			{
		...
		if (bHasStart)
		{
			if (bWantInheritStart)
			{
				...
				m_data.OffsetTaskDate(dwNewTaskID, TDCD_STARTDATE, nOffsetDays, TDCO_DAYS, FALSE, FALSE);
			...
			else // offset children
			{
				...
				m_data.OffsetTaskDate(dwNewTaskID, TDCD_STARTDATE, nOffsetDays, TDCO_DAYS, TRUE, FALSE);
			}
		}
	}
	else // dtNext is the new start date
	{
		...
		if (bHasDue)
		{
			if (bWantInheritDue)
			{
				...
				m_data.OffsetTaskDate(dwNewTaskID, TDCD_DUEDATE, nOffsetDays, TDCO_DAYS, FALSE, FALSE);
				m_data.ApplyLastChangeToSubtasks(dwNewTaskID, TDCA_DUEDATE);
			}
			else // bump
			{
...
BOOL CToDoCtrl::SetSelectedTaskDone(const COleDateTime& date, BOOL bDateEdited)
{
	...
	while (pos)
	{
		...
		int nItemRes = SetTaskDone(hti, dtDone, bAndSubtasks, TRUE);
	...
	if (nRes == SET_CHANGE)
	{
		...
		else if (m_aRecreateTaskIDs.GetSize())
		{
		...
		// if some were completed we may also need to report a status change
...
LRESULT CToDoCtrl::OnRecreateRecurringTask(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	for (int nTask = 0; nTask < nNumTasks; nTask++)
	{
		...
		CTaskFile task;
		VERIFY(AddTreeItemToTaskFile(hti, task, NULL, TDCGT_ALL));
		...
		SetTaskDone(htiNew, 0.0, TDC_SETUNDONE, TRUE);
...
int CToDoCtrl::SetTaskDone(HTREEITEM hti, const COleDateTime& date, 
						   BOOL bAndSubtasks, BOOL bUpdateSubtaskDates)
{
	DWORD dwTaskID = GetTaskID(hti);
	int nRes = 0;
	...
	if (bDoneChange || bUpdateSubtaskDates)
	{
	...
	if (bAndSubtasks)
	{
		...
		while (htiChild)
		{
			if (SetTaskDone(htiChild, date, TRUE, bUpdateSubtaskDates) == SET_CHANGE)
				nRes = SET_CHANGE;
...
{ 
	if (!m_taskTree.GetSelectedTaskRecurrence(tr))
	{
		...
		ASSERT(tr.nRegularity == TDIR_ONCE);
	...
	if (htiNew)
	{
		...
		if (!bRootParent)
		{
			...
			if (m_dwTimeTrackTaskID && 
				(m_dwTimeTrackTaskID == dwParentID) && 
				!HasStyle(TDCS_ALLOWPARENTTIMETRACKING))
			{
				EndTimeTracking(TRUE);
			}
...
BOOL CToDoCtrl::SetStyle(TDC_STYLE nStyle, BOOL bOn, BOOL bWantUpdate)
{
	...
	if (bOn != HasStyle(nStyle))
	{
		...
		if (bWantUpdate)
		{
			m_data.ResetAllCachedCalculations();
			UpdateControls();
		}
...
void CToDoCtrl::SetCustomAttributeDefs(const CTDCCustomAttribDefinitionArray& aAttrib)
{
	...
	if (!Misc::MatchAllT(m_aCustomAttribDefs, aAttrib))
	{
		...
		// inform data model of changes
		m_data.ResetAllCustomCachedCalculations();
...
{
	BOOL bColumnChange, bEditChange, bDummy;
	
	m_visColAttrib.CheckForDiff(vis, bColumnChange, bEditChange, bDummy);
	m_visColAttrib = vis;
...
const CTDCColumnIDArray& CToDoCtrl::GetVisibleColumns() const
{
	return m_visColAttrib.GetVisibleColumns();
}

const CTDCAttributeArray& CToDoCtrl::GetVisibleEditFields() const
{
	return m_visColAttrib.GetVisibleEditFields();
}

const CTDCAttributeArray& CToDoCtrl::GetVisibleFilterFields() const
{
	...
	if (tasks.LoadEx())
	{
		...
		if (tasks.Decrypt())
		{
				...
				HasStyle(TDCS_CHECKOUTONLOAD) && sCheckedOutTo.IsEmpty())
			{
				...
				m_bCheckedOut = tasks.Save(sFilePath);
			}
...
BOOL CToDoCtrl::LoadTasks(const CTaskFile& file)
{
	...
	// Always re-save tasklists as Unicode by default
	m_bIsUnicode = TRUE;

	// And update XML header if not already unicode
	m_sXmlHeader = file.GetXmlHeader();

	if (Misc::ToUpper(m_sXmlHeader).Find(_T("UTF-16")) == -1)
										...
										CMapID2ID& mapID, TDC_RESETIDS nResetID, BOOL bAndSiblings) const
{
	...
	if ((dwTaskID <= 0) || (nResetID == TDCR_YES) || m_data.HasTask(dwTaskID, FALSE))
...
LRESULT CToDoCtrl::OnAutoComboAddDelete(WPARAM wp, LPARAM /*lp*/)
{
	...
	{
	case IDC_FILEPATH:
		// parent doesn't need to know about this
		return 0L;
...
BOOL CToDoCtrl::HandleCustomColumnClick(TDC_COLUMN nColID)
{
	...
	default:
		...
		if ((attribDef.GetListType() == TDCCA_FIXEDLIST) && !attribDef.SupportsCalculation())
		{
			BOOL bNext = (!Misc::IsKeyPressed(VK_SHIFT));
...
BOOL CToDoCtrl::AddSubTasksToTaskFile(const TODOSTRUCTURE* pTDSParent, CTaskFile& tasks, HTASKITEM hParentTask) const
{
   for (int nSubTask = 0; nSubTask < pTDSParent->GetSubTaskCount(); nSubTask++)
   {
      ...
      if (!AddTaskToTaskFile(pTDI, pTDS, tasks, hParentTask))
      {
         ASSERT(0);
         return FALSE;
      }
...
BOOL CToDoCtrl::AddTaskToTaskFile(const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, CTaskFile& tasks, HTASKITEM hParentTask) const
{
...
// 		if (!pTDI->sPath.IsEmpty() && filter.WantAttribute(TDCA_PATH))
// 			file.SetTaskAttribute(hTask, TDL_TASKPATH);
...
LRESULT CToDoCtrl::OnDropObject(WPARAM wParam, LPARAM lParam)
{
	...
	if (bSingleOutlookObj)
	{
		...
		aFiles.Add(COutlookHelper::FormatItemAsUrl(item, OAFMT_NICE));
	}
	else if (nNumFiles)
	{ 
	...
	{
		SetSelectedTaskFileRefs(aFiles, TRUE);
...
CString CToDoCtrl::GetPreferencesKey(const CString& sSubKey) const
{
	...
	{
		sKeyPath = m_sAltPrefsKey;
	}
	else if (!m_sLastSavePath.IsEmpty())
	{
		sKeyPath = m_sLastSavePath;
	}
	// else sKeyPath remains empty

	return GetPreferencesKey(sKeyPath, sSubKey);
}

CString CToDoCtrl::GetPreferencesKey(const CString& sFilePath, const CString& sSubKey)
{
	CString sKey, sKeyPath = (sFilePath.IsEmpty() ? _T("Default") : CPreferences::KeyFromFile(sFilePath));
...
{
	TDCCOLEDITFILTERVISIBILITY vis(m_visColAttrib);

	// columns
	CString sKey = GetPreferencesKey(_T("Columns"));
	...
	if (!sKey.IsEmpty())
	{
		...
		if (nItem >= 0)
		{
			while (nItem--)
			{
				...
				TDC_COLUMN nCol = (TDC_COLUMN)prefs.GetProfileInt(sKey, sItem, TDCC_NONE);
	...
	// attributes
	sKey = GetPreferencesKey();
	TDL_SHOWATTRIB nShow = (TDL_SHOWATTRIB)prefs.GetProfileInt(sKey, _T("ShowEditsAndFilters"), TDLSA_ASCOLUMN);
	...
	if (nShow == TDLSA_ANY)
	{
		...
		if (LoadAttributeVisibility(prefs, GetPreferencesKey(_T("EditFields")), aAttrib))
			vis.SetVisibleEditFields(aAttrib);

		if (LoadAttributeVisibility(prefs, GetPreferencesKey(_T("FilterFields")), aAttrib))
...
int CToDoCtrl::LoadAttributeVisibility(const CPreferences& prefs, LPCTSTR szKey, CTDCAttributeArray& aAttrib)
{
	aAttrib.RemoveAll();
	int nItem = prefs.GetProfileInt(szKey, _T("Count"), -1);
	...
	if (nItem > 0)
	{
		...
		{
			CString sItem = Misc::MakeKey(_T("Item%d"), nItem);
			TDC_ATTRIBUTE nAttrib = (TDC_ATTRIBUTE)prefs.GetProfileInt(szKey, sItem, TDCA_NONE);
...
void CToDoCtrl::SaveAttributeVisibility(CPreferences& prefs) const
{
	...
	if (!sKey.IsEmpty())
	{		
		int nItem = GetVisibleColumns().GetSize();
		...
		{
			CString sItem = Misc::MakeKey(_T("Item%d"), nItem);
			prefs.WriteProfileInt(sKey, sItem, GetVisibleColumns()[nItem]);
		}
	}
	
	// attributes
	TDL_SHOWATTRIB nShow = m_visColAttrib.GetShowEditsAndFilters();
	prefs.WriteProfileInt(GetPreferencesKey(), _T("ShowEditsAndFilters"), nShow);

	if (nShow == TDLSA_ANY)
	{
		SaveAttributeVisibility(prefs, GetPreferencesKey(_T("EditFields")), GetVisibleEditFields());
		SaveAttributeVisibility(prefs, GetPreferencesKey(_T("FilterFields")), GetVisibleFilterFields());
	}
}

void CToDoCtrl::SaveAttributeVisibility(CPreferences& prefs, LPCTSTR szKey, const CTDCAttributeArray& aAttrib)
{
	int nItem = aAttrib.GetSize();
	
	prefs.WriteProfileInt(szKey, _T("Count"), nItem);
	
	while (nItem--)
	{
		CString sItem = Misc::MakeKey(_T("Item%d"), nItem);
		prefs.WriteProfileInt(szKey, sItem, aAttrib[nItem]);
	}
}

void CToDoCtrl::SaveTasksState(CPreferences& prefs) const
{
	m_taskTree.SaveState(prefs, GetPreferencesKey(_T("TaskTree")));
}

HTREEITEM CToDoCtrl::LoadTasksState(const CPreferences& prefs)
{
	return m_taskTree.LoadState(prefs, GetPreferencesKey(_T("TaskTree")));
}
...
{ 
	...
	if (m_taskTree.SelectNextTasksInHistory())
		UpdateControls();
}
...
{ 
	...
	if (m_taskTree.SelectPrevTasksInHistory())
		UpdateControls();
}
...
BOOL CToDoCtrl::GotoFile(const CString& sFile, BOOL bShellExecute)
{
	...
	return GetParent()->SendMessage(WM_TDCM_FAILEDLINK, 0, (LPARAM)(LPCTSTR)sFile);
}
...
LRESULT CToDoCtrl::OnTDCFailedLink(WPARAM wParam, LPARAM lParam)
{
	...
	return GetParent()->SendMessage(WM_TDCM_FAILEDLINK, wParam, lParam);
}
...
// 				{
// 					// insert as top child
// 					if (m_taskTree.ItemHasChildren(htiDestParent))
// 					{
// 						htiTarget = m_taskTree.GetChildItem(htiDestParent);
// 						nWhere = TCHW_ABOVE;
// 					}
// 					else
// 					{
...
LRESULT CToDoCtrl::OnTaskHasReminder(WPARAM wp, LPARAM lp)
{
...
BOOL CToDoCtrl::TaskHasReminder(DWORD dwTaskID) const
{
	return (GetParent()->SendMessage(WM_TDCM_TASKHASREMINDER, dwTaskID, (LPARAM)this) ? 1 : 0);
}
...
BOOL CToDoCtrl::RedrawReminders() const
{ 
	if (IsColumnShowing(TDCC_REMINDER))
	{
		m_taskTree.RedrawColumn(TDCC_REMINDER);
		return TRUE;
	}
