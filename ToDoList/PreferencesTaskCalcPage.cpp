   ...
   CPreferencesPageBase(CPreferencesTaskCalcPage::IDD)
{
	...
	m_bSetCompletionStatus = FALSE;
	m_sCompletionStatus = _T("");
	//}}AFX_DATA_INIT
...
void CPreferencesTaskCalcPage::LoadPreferences(const CPreferences& prefs)
{
	...
	m_nRecentModTimeUnits = (TCHAR)prefs.GetProfileInt(_T("Preferences"), _T("RecentModTimeUnits"), THU_HOURS);
...
void CPreferencesTaskCalcPage::SavePreferences(CPreferences& prefs)
{
	...
	// cleanup old entry	
	prefs.DeleteEntry(_T("Preferences"), _T("UseEarliestDueDate"));
