...
TDL_WEBUPDATE_CHECK CTDLWebUpdatePromptDlg::CheckForUpdates(BOOL bStaging, LPCTSTR szCurVer)
{
	if (!WebMisc::IsOnline())
		return TDLWUC_NOTCONNECTED;

	// download the update script to temp file
	CTDCWebUpdateScript script(bStaging);
	CWaitCursor wait;

	// first for pre-release update
	BOOL bPreReleaseUpdates = script.CheckForPreReleaseUpdates(szCurVer);
	BOOL bUpdates = (bPreReleaseUpdates || script.CheckForUpdates(szCurVer));
