...
BOOL CTDCImageList::LoadImages(const CString& sTaskList, COLORREF crTransparent, BOOL bWantDefaultIcons)
{
	...
	if (Create(16, 16, ILC_COLOR32 | ILC_MASK, 0, 200))
	{
		...
		ASSERT (hIcon);
		Add(hIcon);
		m_mapNameToIndex[CString("0")] = 0;
		m_mapIndexToName[0] = CString("0");
		...
		if (bWantDefaultIcons)
		{
			...
			if (FileMisc::FileExists(sTaskList))
				dwResult = LoadImagesFromFolder(FileMisc::GetFolderFromFilePath(sTaskList), crTransparent, NULL);

			// else try application location
			if (!dwResult)
				dwResult = LoadImagesFromFolder(FileMisc::GetAppResourceFolder(), crTransparent, NULL);
			...
			if ((dwResult & TDCIL_LOADEDSET) == 0)
			{
				...
				if (bm.LoadBitmap(IDB_TASKICONS_STD))
					AddImage("", bm, crTransparent, this);
			}
		}
		...
		if (FileMisc::FileExists(sTaskList))
		{
			dwResult = LoadImagesFromFolder(FileMisc::GetFolderFromFilePath(sTaskList), crTransparent, this);
		}

		// else try application location
		if (!dwResult)
			dwResult = LoadImagesFromFolder(FileMisc::GetAppResourceFolder(), crTransparent, this);
	}
