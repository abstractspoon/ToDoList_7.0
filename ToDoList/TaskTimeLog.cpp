...
CTDCTaskTimeLog::CTDCTaskTimeLog(LPCTSTR szRefPath, SFE_SAVEAS nSaveAs) 
	: 
...
BOOL CTDCTaskTimeLog::LogTime(const TASKTIMELOGITEM& li, BOOL bLogSeparately)
{
	...
	// column headings as the first line and save as UTF-8
	if (!m_bLogExists)
	{
		CString sHeader;
		sHeader.Format(_T("%s %d\n%s\n"), HEADER_LINE, m_nVersion, GetLatestColumnHeader());

		VERIFY(FileMisc::SaveFile(sLogPath, sHeader, SFE_MULTIBYTE, CP_UTF8));
	}

	return FileMisc::AppendLineToFile(sLogPath, li.FormatRow(m_nVersion, m_sDelim), SFE_ASIS);
}
...
int CTDCTaskTimeLog::ReadLogItems(const CString& sLogPath, CTaskTimeLogItemArray& aLogItems, BOOL bAppend, CString& sDelim)
{
	...
	{
		CStringArray aLines;
		int nNumLines = FileMisc::LoadFileLines(sLogPath, aLines);
	...
	case TTLF_CSV:
		{
			...
			if (file.Open(szOutputFile, CFile::modeCreate | CFile::modeWrite | CFile::typeText))
			{
				// header row
...
BOOL CTDCTaskTimeLogAnalysis::IsSamePeriod(const COleDateTime& date1, const COleDateTime& date2, TDCTTL_BREAKDOWN nBreakdown)
{
	...
	case TTLB_BYWEEK:
		return (CDateHelper::CalcWeekofYear(date1) == CDateHelper::CalcWeekofYear(date2));
