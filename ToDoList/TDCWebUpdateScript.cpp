...
const LPCTSTR STAGING_SCRIPT_URL	= _T("http://www.abstractspoon.com/todolist_staging_new.txt");
...
CTDCWebUpdateScript::CTDCWebUpdateScript(BOOL bStaging) : m_bStaging(bStaging)
{
...
BOOL CTDCWebUpdateScript::CheckForPreReleaseUpdates(LPCTSTR szCurVer)
{
	return CheckForUpdates(szCurVer, TRUE);
}

BOOL CTDCWebUpdateScript::CheckForUpdates(LPCTSTR szCurVer)
{
	return CheckForUpdates(szCurVer, FALSE);
}

BOOL CTDCWebUpdateScript::CheckForUpdates(LPCTSTR szCurVer, BOOL bPreRelease)
{
	if (!LoadScript(GetScriptUrl(bPreRelease)))
		return FALSE;

	CString sExeVer, sDisplayVer;
	
	if (!GetExeVersion(sExeVer))
	{
	...
	CString sCurVer(szCurVer);
	
	if (sCurVer.IsEmpty())
		sCurVer = FileMisc::GetModuleVersion();

	// NOTE: only allow updates to pre-releases 
	// if the app already is one of these versions
	if (bPreRelease)
	{
		if (IsPreRelease(sExeVer) && !IsPreRelease(sCurVer))
			return FALSE;
	}
	
	// else simple compare
	return (FileMisc::CompareVersions(sCurVer, sExeVer, 4) < 0);
}
...
BOOL CTDCWebUpdateScript::LoadScript(const CString& sScriptUrl)
{
	...
	CStringArray aLines;
	int nLine = FileMisc::LoadFileLines(sTempFile, aLines);
...
{
	return (GetExeVersion(sExeVer) && GetDisplayVersion(sDisplayVer) && GetDescription(aDescription));
}
...
BOOL CTDCWebUpdateScript::GetExeVersion(CString& sVer) const
{
	return m_mapScript.Lookup(EXE_VER_KEY, sVer);
}

BOOL CTDCWebUpdateScript::GetDisplayVersion(CString& sVer) const
{
	if (m_mapScript.Lookup(DISPLAY_VER_KEY, sVer) && !sVer.IsEmpty())
		return TRUE;

	// else
	return GetExeVersion(sVer);
}

int CTDCWebUpdateScript::GetDescription(CStringArray& aDesc) const
{
	CString sDesc;
