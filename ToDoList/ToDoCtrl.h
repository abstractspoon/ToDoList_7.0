...
public:
	...
	int GetAutoListData(TDCAUTOLISTDATA& tld, BOOL bIncDefault = TRUE) const;
	...
	double GetSelectedTaskTimeEstimate(TCHAR& nUnits) const { return m_taskTree.GetSelectedTaskTimeEstimate(nUnits); }
	double GetSelectedTaskTimeSpent(TCHAR& nUnits) const { return m_taskTree.GetSelectedTaskTimeSpent(nUnits); }
	...
	double CalcSelectedTaskTimeEstimate(TCHAR nUnits = TDCU_HOURS) const { return m_taskTree.CalcSelectedTaskTimeEstimate(nUnits); }
	double CalcSelectedTaskTimeSpent(TCHAR nUnits = TDCU_HOURS) const { return m_taskTree.CalcSelectedTaskTimeSpent(nUnits); }
	...
	BOOL SetSelectedTaskTimeEstimate(double dTime, TCHAR nUnits = TDCU_HOURS);
	BOOL SetSelectedTaskTimeSpent(double dTime, TCHAR nUnits = TDCU_HOURS);
	...
	BOOL SetSelectedTaskFileRef(const CString& sFilePath, BOOL bAppend = FALSE);
	...
	BOOL CanSelectNextTasksInHistory() const { return m_taskTree.CanSelectNextTasksInHistory(); }
	void SelectNextTasksInHistory();
	BOOL CanSelectPrevTasksInHistory() const { return m_taskTree.CanSelectPrevTasksInHistory(); }
...
protected:
	...
	int SetTaskDone(HTREEITEM hti, const COleDateTime& date, BOOL bAndSubtasks, BOOL bUpdateSubtaskDates = TRUE);
	...
	virtual void SaveTasksState(CPreferences& prefs) const; // keyed by last filepath
	virtual HTREEITEM LoadTasksState(const CPreferences& prefs); // returns the previously selected item if any
	...
	BOOL SelectTask(DWORD dwTaskID, BOOL bTrue);
	BOOL SelectTasks(const CDWordArray& aTaskIDs, BOOL bTrue);
