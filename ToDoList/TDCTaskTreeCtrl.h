...
class CTDCTaskTreeCtrl : public CTDCTaskCtrlBase  
{
	DECLARE_DYNAMIC(CTDCTaskTreeCtrl);

public:
	CTDCTaskTreeCtrl(const CTDCImageList& ilIcons,
					const CToDoCtrlData& data, 
					const CWordArray& aStyles,
					const CTDCColumnIDArray& aVisibleCols,
					const CTDCCustomAttribDefinitionArray& aCustAttribDefs);

	virtual ~CTDCTaskTreeCtrl();
	...
	inline const TODOITEM* GetTask(DWORD dwTaskID) const { return CTDCTaskCtrlBase::GetTask(dwTaskID); }
	...
	BOOL SelectNextTasksInHistory();
	BOOL SelectPrevTasksInHistory();

	double CalcSelectedTaskTimeEstimate(TCHAR nUnits = TDCU_HOURS) const;
	double CalcSelectedTaskTimeSpent(TCHAR nUnits = TDCU_HOURS) const;
	...
	HTREEITEM LoadState(const CPreferences& prefs, const CString& sKey);
	...
	BOOL CanSelectNextTasksInHistory() const { return TSH().HasNextSelection(); }
	BOOL CanSelectPrevTasksInHistory() const { return TSH().HasPrevSelection(); }
...
protected:
	...
	//{{AFX_VIRTUAL(CTDCTreeListCtrl)
	//}}AFX_VIRTUAL
	
protected:
	// Generated message map functions
	//{{AFX_MSG(CTDCTreeListCtrl)
	//}}AFX_MSG
