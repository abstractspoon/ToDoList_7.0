...
BOOL TDCSTARTUPATTRIB::GetTime(double& dValue, int& nUnits, BOOL& bOffset) const
{
	if (!bSet)
		return FALSE;
	...
	{
		bOffset = TRUE;
		return CTimeHelper::DecodeOffset(szValue, dValue, nUnits, TRUE);
	}

	// else
	bOffset = FALSE;
	return CTimeHelper::DecodeOffset(szValue, dValue, nUnits, FALSE);
}

BOOL TDCSTARTUPATTRIB::GetDate(double& dValue, int& nUnits, BOOL& bOffset) const
{
	if (!bSet)
		return FALSE;
	...
	{
		bOffset = TRUE;
		return CDateHelper::DecodeOffset(szValue, dValue, nUnits, TRUE);
	}

	// else
	bOffset = FALSE;
	return CDateHelper::DecodeOffset(szValue, dValue, nUnits, FALSE);
}
