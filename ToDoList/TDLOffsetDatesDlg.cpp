...
CTDLOffsetDatesDlg::CTDLOffsetDatesDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CTDLOffsetDatesDlg::IDD, pParent)
{
	...
	m_nOffsetByUnits = prefs.GetProfileInt(_T("OffsetDates"), _T("AmountPeriod"), -1);

	// backwards compatibility
	if (m_nOffsetByUnits == -1)
	{
		m_nOffsetByUnits = (prefs.GetProfileInt(_T("OffsetDates"), _T("AmountUnits"), 0) + 1);
		prefs.DeleteEntry(_T("OffsetDates"), _T("AmountUnits"));
...
BEGIN_MESSAGE_MAP(CTDLOffsetDatesDlg, CDialog)
