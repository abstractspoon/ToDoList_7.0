...
IMPLEMENT_DYNAMIC(CTDCTaskTreeCtrl, CTDCTaskCtrlBase)

//////////////////////////////////////////////////////////////////////

CTDCTaskTreeCtrl::CTDCTaskTreeCtrl(const CTDCImageList& ilIcons,
								   const CToDoCtrlData& data, 
								   const CWordArray& aStyles,
								   const CTDCColumnIDArray& aVisibleCols,
								   const CTDCCustomAttribDefinitionArray& aCustAttribDefs) 
	: 
	CTDCTaskCtrlBase(FALSE, ilIcons, data, m_find, aStyles, aVisibleCols, aCustAttribDefs),
	m_tsh(m_tcTasks),
	m_tch(m_tcTasks),
...
CTDCTaskTreeCtrl::~CTDCTaskTreeCtrl()
{
}

///////////////////////////////////////////////////////////////////////////

BEGIN_MESSAGE_MAP(CTDCTaskTreeCtrl, CTDCTaskCtrlBase)
...
BOOL CTDCTaskTreeCtrl::CreateTasksWnd(CWnd* pParentWnd, const CRect& rect, BOOL bVisible)
{
...
BOOL CTDCTaskTreeCtrl::IsTreeItemSelected(HWND hwnd, HTREEITEM hti) const
{
...
// external version
BOOL CTDCTaskTreeCtrl::SelectItem(HTREEITEM hti) 
{ 
	return SelectItem(hti, TRUE, SC_UNKNOWN);
}

// internal version) 
BOOL CTDCTaskTreeCtrl::SelectItem(HTREEITEM hti, BOOL bSyncAndNotify, SELCHANGE_ACTION nBy)
{ 
...
void CTDCTaskTreeCtrl::DeselectAll()
{
	// prevent resyncing
...
BOOL CTDCTaskTreeCtrl::SelectAll() 
{ 
	if (TSH().AddAll())
	{
		SyncColumnSelectionToTasks();
...
void CTDCTaskTreeCtrl::OnStylesUpdated()
{
	CTDCTaskCtrlBase::OnStylesUpdated();

	SyncColumnSelectionToTasks();
}

void CTDCTaskTreeCtrl::OnStyleUpdated(TDC_STYLE nStyle, BOOL bOn, BOOL bDoUpdate)
{
	// base class
...
BOOL CTDCTaskTreeCtrl::BuildColumns()
{
	// nothing more to do
	return CTDCTaskCtrlBase::BuildColumns();
}

DWORD CTDCTaskTreeCtrl::HitTestTasksTask(const CPoint& ptScreen) const
{
...
int CTDCTaskTreeCtrl::GetExpandedTasks(CDWordArray& aExpanded) const
{
	return GetExpandedTasks(aExpanded, NULL);
}

int CTDCTaskTreeCtrl::GetExpandedTasks(CDWordArray& aExpanded, HTREEITEM hti) const
{
	int nStart = 0;
...
void CTDCTaskTreeCtrl::SetExpandedTasks(const CDWordArray& aExpanded)
{
...
BOOL CTDCTaskTreeCtrl::EnsureSelectionVisible()
{
	if (GetSelectedCount())
	{
...
BOOL CTDCTaskTreeCtrl::SelectNextTasksInHistory() 
{ 
	if (TSH().NextSelection())
	{
		InvalidateAll();
		return TRUE;
	}
...
BOOL CTDCTaskTreeCtrl::SelectPrevTasksInHistory() 
{ 
...
BOOL CTDCTaskTreeCtrl::IsColumnShowing(TDC_COLUMN nColID) const
{
	// never show path
	if (nColID == TDCC_PATH)
		return FALSE;
	...
	// else
	return CTDCTaskCtrlBase::IsColumnShowing(nColID);
}

BOOL CTDCTaskTreeCtrl::IsTreeItemLineOdd(HTREEITEM hti) const
{
	return IsColumnLineOdd(GetListItem(hti));
}

int CTDCTaskTreeCtrl::GetListItem(HTREEITEM hti) const
{
	return FindListItem(m_lcColumns, (DWORD)hti);
}

void CTDCTaskTreeCtrl::DeleteAll()
{
	TSH().RemoveAll();
...
void CTDCTaskTreeCtrl::ExpandAll(BOOL bExpand)
{
	...
	if (!bExpand)
	{
		...
		if (htiParent != htiSel)
			SelectItem(htiParent, FALSE, CTDCTaskCtrlBase::SC_UNKNOWN);
	}

	ExpandItem(NULL, bExpand, TRUE);
}

void CTDCTaskTreeCtrl::ExpandItem(HTREEITEM hti, BOOL bExpand, BOOL bAndChildren)
{
...
BOOL CTDCTaskTreeCtrl::CanExpandItem(HTREEITEM hti, BOOL bExpand) const
{
...
void CTDCTaskTreeCtrl::ResortSelectedTaskParents()
{
	CHTIList lstParents;
...
LRESULT CTDCTaskTreeCtrl::OnTreeCustomDraw(NMTVCUSTOMDRAW* pTVCD) 
{
...
BOOL CTDCTaskTreeCtrl::GetSelectionBoundingRect(CRect& rSelection) const
{
...
GM_ITEMSTATE CTDCTaskTreeCtrl::GetTreeItemState(HTREEITEM hti) const
{
...
GM_ITEMSTATE CTDCTaskTreeCtrl::GetColumnItemState(int nItem) const
{
	return GetTreeItemState((HTREEITEM)m_lcColumns.GetItemData(nItem));
}

void CTDCTaskTreeCtrl::OnListSelectionChange(NMLISTVIEW* pNMLV)
{
...
void CTDCTaskTreeCtrl::SyncColumnSelectionToTasks()
{
...
void CTDCTaskTreeCtrl::NotifyParentSelChange(SELCHANGE_ACTION nAction)
{
	NMTREEVIEW nmtv = { 0 };
...
void CTDCTaskTreeCtrl::OnTreeSelectionChange(NMTREEVIEW* pNMTV)
{
...
BOOL CTDCTaskTreeCtrl::PreTranslateMessage(MSG* pMsg)
{
	switch (pMsg->message)
	{
	case WM_NCMOUSEMOVE:
	...
	return CTDCTaskCtrlBase::PreTranslateMessage(pMsg);
}

BOOL CTDCTaskTreeCtrl::HandleClientColumnClick(const CPoint& pt, BOOL bDblClk)
{
...
BOOL CTDCTaskTreeCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
LRESULT CTDCTaskTreeCtrl::OnTreeGetDispInfo(NMTVDISPINFO* pTVDI)
{
...
void CTDCTaskTreeCtrl::Trace(LPCTSTR szComment)
{
	Trace(szComment, NULL);
}

void CTDCTaskTreeCtrl::Trace(LPCTSTR szComment, HTREEITEM hti)
{
...
void CTDCTaskTreeCtrl::Trace(HTREEITEM hti, int nLevel)
{
	if (hti && (hti != TVI_ROOT))
	{
...
LRESULT CTDCTaskTreeCtrl::WindowProc(HWND hRealWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	if (!IsResyncEnabled())
...
void CTDCTaskTreeCtrl::BeginLabelEditTimer()
{
	if (!m_bEditLabelTimerStarted)
	{
		m_bEditLabelTimerStarted = TRUE;

		TRACE(_T("CTDCTaskTreeCtrl::BeginLabelEditTimer()\n"));
		SetTimer(TIMER_EDITLABEL, 500, NULL);
	}
}

void CTDCTaskTreeCtrl::EndLabelEditTimer()
{
	if (m_bEditLabelTimerStarted)
	{
		m_bEditLabelTimerStarted = FALSE;

		TRACE(_T("CTDCTaskTreeCtrl::EndLabelEditTimer()\n"));
		KillTimer(TIMER_EDITLABEL);
	}
}

LRESULT CTDCTaskTreeCtrl::ScWindowProc(HWND hRealWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	if (!IsResyncEnabled())
...
void CTDCTaskTreeCtrl::ExpandList(HTREEITEM hti)
{
	int nNextIndex = 0;
...
void CTDCTaskTreeCtrl::ExpandList(HTREEITEM hti, int& nNextIndex)
{
	CTDCTaskCtrlBase::ExpandList(m_lcColumns, m_tcTasks, hti, nNextIndex);
}

void CTDCTaskTreeCtrl::CollapseList(HTREEITEM hti)
{
	CTDCTaskCtrlBase::CollapseList(m_lcColumns, m_tcTasks, hti);
}

void CTDCTaskTreeCtrl::GetWindowRect(CRect& rWindow, BOOL bWithHeader) const
{
	// always
...
DWORD CTDCTaskTreeCtrl::GetColumnItemTaskID(int nItem) const
{
	return GetTaskID((HTREEITEM)m_lcColumns.GetItemData(nItem));
}

int CTDCTaskTreeCtrl::RecalcColumnWidth(int nCol, CDC* pDC) const
{
	return CTDCTaskCtrlBase::RecalcColumnWidth(nCol, pDC, TRUE);
}

BOOL CTDCTaskTreeCtrl::GetItemTitleRect(HTREEITEM hti, TDC_TITLERECT nArea, CRect& rect, CDC* pDC, LPCTSTR szTitle) const
{
	ASSERT(hti);
...
BOOL CTDCTaskTreeCtrl::GetLabelEditRect(CRect& rLabel) const
{
	return GetItemTitleRect(GetSelectedItem(), TDCTR_EDIT, rLabel);
}

BOOL CTDCTaskTreeCtrl::RemoveOrphanTreeItemReferences(HTREEITEM hti)
{
	BOOL bRemoved = FALSE;
	...
	while (htiChild)
	{
		...
		if (!m_data.HasTask(dwTaskID, FALSE))
		{
...
HTREEITEM CTDCTaskTreeCtrl::MoveItem(HTREEITEM hti, HTREEITEM htiDestParent, HTREEITEM htiDestPrevSibling)
{
...
BOOL CTDCTaskTreeCtrl::MoveSelection(TDC_MOVETASK nDirection)
{
...
void CTDCTaskTreeCtrl::MoveSelection(HTREEITEM htiDestParent, HTREEITEM htiDestPrevSibling)
{
...
BOOL CTDCTaskTreeCtrl::CanMoveSelection(TDC_MOVETASK nDirection) const
{
	if (IsReadOnly())
		return FALSE;
...
BOOL CTDCTaskTreeCtrl::CanMoveItem(HTREEITEM hti, TDC_MOVETASK nDirection) const
{
	if (!hti)
		return FALSE;
...
BOOL CTDCTaskTreeCtrl::InvalidateSelection(BOOL bUpdate)
{
...
BOOL CTDCTaskTreeCtrl::InvalidateTask(DWORD dwTaskID, BOOL bUpdate)
{
	if (dwTaskID == 0)
...
BOOL CTDCTaskTreeCtrl::InvalidateItem(HTREEITEM hti, BOOL bUpdate)
{
	// draw tree first
	if (TCH().InvalidateItem(hti))
	{
		...
		return CTDCTaskCtrlBase::InvalidateColumnItem(nItem, bUpdate);
	}
...
void CTDCTaskTreeCtrl::CacheSelection(TDCSELECTIONCACHE& cache) const
{
...
BOOL CTDCTaskTreeCtrl::RestoreSelection(const TDCSELECTIONCACHE& cache)
{
	if (cache.aSelTaskIDs.GetSize())
	{
		...
		if (GetItem(dwFocusedTaskID) == NULL)
		{
			dwFocusedTaskID = 0;
		...
		if (SelectTasks(cache.aSelTaskIDs))
		{
			// restore focused item
			HTREEITEM htiFocus = FindTreeItem(m_tcTasks, NULL, dwFocusedTaskID);
			ASSERT(htiFocus);
			...
			{
				HTREEITEM htiFirstVis = GetItem(cache.dwFirstVisibleTaskID);
...
int CTDCTaskTreeCtrl::GetSelectedTaskIDs(CDWordArray& aTaskIDs, BOOL bTrue) const
{
	DWORD dwFocusID;
...
int CTDCTaskTreeCtrl::GetSelectedTaskIDs(CDWordArray& aTaskIDs, DWORD& dwFocusedTaskID, BOOL bRemoveChildDupes) const
{
	if (!GetSelectedCount())
		return 0;
...
BOOL CTDCTaskTreeCtrl::MultiSelectTask(DWORD dwTaskID, BOOL bSelected)
{
	if (dwTaskID == 0)
		return FALSE;
...
BOOL CTDCTaskTreeCtrl::SelectTasks(const CDWordArray& aTaskIDs, BOOL bTrue)
{
	// sanity check
...
BOOL CTDCTaskTreeCtrl::MultiSelectItem(HTREEITEM hti, TSH_SELECT nState, BOOL bRedraw) 
{ 
...
BOOL CTDCTaskTreeCtrl::MultiSelectItems(HTREEITEM htiFrom, HTREEITEM htiTo, TSH_SELECT nState, BOOL bRedraw)
{
...
DWORD CTDCTaskTreeCtrl::GetTaskParentID(HTREEITEM hti) const
{
...
HTREEITEM  CTDCTaskTreeCtrl::InsertItem(DWORD dwTaskID, HTREEITEM htiParent, HTREEITEM htiAfter)
{
...
BOOL CTDCTaskTreeCtrl::GetInsertLocation(TDC_MOVETASK nDirection, DWORD& dwDest, DWORD& dwDestAfter) const
{
...
BOOL CTDCTaskTreeCtrl::GetInsertLocation(TDC_MOVETASK nDirection, HTREEITEM& htiDest, HTREEITEM& htiDestAfter) const
{
...
BOOL CTDCTaskTreeCtrl::GetInsertLocation(TDC_INSERTWHERE nWhere, HTREEITEM& htiDest, HTREEITEM& htiDestAfter) const
{
	HTREEITEM htiSel = GetSelectedItem();
	htiDest = htiDestAfter = NULL;
...
void CTDCTaskTreeCtrl::SaveState(CPreferences& prefs, const CString& sKey) const
{
	CTDCTaskCtrlBase::SaveState(prefs, sKey);
...
HTREEITEM CTDCTaskTreeCtrl::LoadState(const CPreferences& prefs, const CString& sKey) 
{
	CTDCTaskCtrlBase::LoadState(prefs, sKey);
...
int CTDCTaskTreeCtrl::SaveTreeExpandedState(CPreferences& prefs, const CString& sKey, 
...
void CTDCTaskTreeCtrl::SetTasksImageList(HIMAGELIST hil, BOOL bState, BOOL bOn)
{
	if (bOn)
...
double CTDCTaskTreeCtrl::CalcSelectedTaskTimeEstimate(TCHAR nUnits) const
{
	double dTime = 0.0;
...
double CTDCTaskTreeCtrl::CalcSelectedTaskTimeSpent(TCHAR nUnits) const
{
	double dTime = 0.0;
...
double CTDCTaskTreeCtrl::CalcSelectedTaskCost() const
{
	double dCost = 0.0;
