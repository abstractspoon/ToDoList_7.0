													   ...
													   CWnd* pParent, UINT nCtrlID, BOOL bVisible)
{
	...
	if (pControl)
	{
		...
		else 
		{
			...
			pControl->SendMessage(WM_SETFONT, (WPARAM)hFont); 
															...
															CWnd* pParent, UINT nCtrlID, BOOL bVisible)
{
	...
	if (pLabel)
	{
		...
		else // font
		{
			...
			// THEN set label
			pLabel->SetWindowText(attrib.sLabel.IsEmpty() ? attrib.sColumnTitle : attrib.sLabel);
