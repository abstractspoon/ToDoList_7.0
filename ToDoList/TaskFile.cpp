...
BOOL CTaskFile::SetAttributeVisibility(const TDCCOLEDITFILTERVISIBILITY& vis)
{
	...
	// columns
	const CTDCColumnIDArray& aCols = vis.GetVisibleColumns();
	int nVis = aCols.GetSize();

	while (nVis--)
		pXIVis->AddItem(TDL_ATTRIBVISCOL, aCols[nVis], XIT_ELEMENT);
	...
	{
		// edit fields
		const CTDCAttributeArray& aEdit = vis.GetVisibleEditFields();
		nVis = aEdit.GetSize();

		while (nVis--)
			pXIVis->AddItem(TDL_ATTRIBVISEDIT, aEdit[nVis], XIT_ELEMENT);

		// filter fields
		const CTDCAttributeArray& aFilter = vis.GetVisibleFilterFields();
		nVis = aFilter.GetSize();

		while (nVis--)
			pXIVis->AddItem(TDL_ATTRIBVISFILTER, aFilter[nVis], XIT_ELEMENT);
	}
...
BOOL CTaskFile::GetAttributeVisibility(TDCCOLEDITFILTERVISIBILITY& vis) const
{
	...
	while (pXICol)
	{
		aCols.Add(TDC_COLUMN(pXICol->GetValueI()));
		pXICol = pXICol->GetSibling();
	}

	vis.SetVisibleColumns(aCols);
	...
	if (vis.GetShowEditsAndFilters() == TDLSA_ANY)
	{
		...
		CTDCAttributeArray aEdit;

		while (pXIEdit)
		{
			aEdit.Add(TDC_ATTRIBUTE(pXIEdit->GetValueI()));
			pXIEdit = pXIEdit->GetSibling();
		}

		// filter fields
		const CXmlItem* pXIFilter = pXIVis->GetItem(TDL_ATTRIBVISFILTER);
		CTDCAttributeArray aFilter;

		while (pXIFilter)
		{
			aFilter.Add(TDC_ATTRIBUTE(pXIFilter->GetValueI()));
			pXIFilter = pXIFilter->GetSibling();
		}

		vis.SetVisibleEditFields(aEdit);
		vis.SetVisibleFilterFields(aFilter);
	}
...
bool CTaskFile::AddCustomAttribute(LPCTSTR szID, LPCTSTR szLabel)
{
	...
	while (pXIAttribDef)
	{
		if (pXIAttribDef->GetItemValue(TDL_CUSTOMATTRIBID) == szID)
			return false;
...
{
	return (GetTaskUChar(hTask, TDL_TASKGOODASDONE) != 0);
}
...
BOOL CTaskFile::GetTaskRecurrence(HTASKITEM hTask, TDIRECURRENCE& tr) const
{
	...
	return GetTaskRecurrence(hTask, 
							(int&)tr.nRegularity, 
							tr.dwSpecific1, 
							tr.dwSpecific2, 
							(int&)tr.nRecalcFrom, 
									...
									DWORD& dwSpecific2, BOOL& bRecalcFromDue, int& nReuse) const
{
	...
	return GetTaskRecurrence(hTask, nRegularity, dwSpecific1, dwSpecific2, bRecalcFromDue, nReuse, nDummy);
}
...
bool CTaskFile::SetTaskCustomAttributeData(HTASKITEM hTask, LPCTSTR szID, LPCTSTR szData)
{
	...
	if (!HasCustomAttribute(szID))
	{
		...
		pXIAttribDef->SetItemValue(TDL_CUSTOMATTRIBID, szID);
		pXIAttribDef->SetItemValue(TDL_CUSTOMATTRIBLABEL, szID);
		pXIAttribDef->SetItemValue(TDL_CUSTOMATTRIBCOLTITLE, szID);
		pXIAttribDef->SetItemValue(TDL_CUSTOMATTRIBENABLED, TRUE);
	}
...
bool CTaskFile::SetTaskCreationDate64(HTASKITEM hTask, time64_t tDate)
{
	...
	return SetTaskDate(hTask, TDL_TASKCREATIONDATE, date, TRUE);
}
...
bool CTaskFile::SetTaskLastModified64(HTASKITEM hTask, time64_t tDate)
{
	...
	return SetTaskDate(hTask, TDL_TASKLASTMOD, date, TRUE);
}
...
bool CTaskFile::SetTaskDoneDate64(HTASKITEM hTask, time64_t tDate)
{
	...
	return SetTaskDate(hTask, TDL_TASKDONEDATE, date, TRUE);
}
...
bool CTaskFile::SetTaskDueDate64(HTASKITEM hTask, time64_t tDate)
{
	...
	return SetTaskDate(hTask, TDL_TASKDUEDATE, date, TRUE);
}
...
bool CTaskFile::SetTaskStartDate64(HTASKITEM hTask, time64_t tDate)
{
	...
	return SetTaskDate(hTask, TDL_TASKSTARTDATE, date, TRUE);
}
...
BOOL CTaskFile::SetTaskRecurrence(HTASKITEM hTask, const TDIRECURRENCE& tr)
{
	if (SetTaskRecurrence(hTask, 
							...
							tr.nNumRecur))
	{
		SetTaskString(hTask, TDL_TASKRECURRENCE, tr.GetRegularity(), XIT_ELEMENT); // human readable
		return true;
	}
...
{
	return SetTaskDate(hTask, TDL_TASKCALCDUEDATE, date, FALSE, TDL_TASKCALCDUEDATESTRING);
}

BOOL CTaskFile::SetTaskCalcStartDate(HTASKITEM hTask, const COleDateTime& date)
{
	return SetTaskDate(hTask, TDL_TASKCALCSTARTDATE, date, FALSE, TDL_TASKCALCSTARTDATESTRING);
}
...
{
	COleDateTime dtTask = (bIncTime ? date : CDateHelper::GetDateOnly(date));
	
	if (SetTaskDouble(hTask, sDateItem, dtTask))
	{
		if (!sDateStringItem.IsEmpty())
		{
			DWORD dwFmt = m_bISODates ? DHFD_ISO : 0;
			
			if (bIncTime && CDateHelper::DateHasTime(date))
