...
void CiCalExporter::ExportTask(const ITaskList12* pTasks, HTASKITEM hTask, const CString& sParentUID, CStdioFile& fileOut, BOOL bAndSiblings)
{
	if (!hTask)
		return;
	...
	int nPercent = pTasks->GetTaskPercentDone(hTask, FALSE);
	...
	if (bStartDate || bEndDate)
	{
		...
		if (bDueDate)
			WriteString(fileOut, FormatDateTime(_T("DUE"), CDateHelper::GetDate(tDue), FALSE));
		...
		WriteString(fileOut, _T("PERCENT:%lu"), nPercent);
...
CString CiCalExporter::FormatRecurrence(int nRegularity, DWORD dwSpecific1, DWORD dwSpecific2)
{
	...
	case TDIR_WEEK_SPECIFIC_DOWS_NMONTHS: // TDIR_WEEKLY
		{
