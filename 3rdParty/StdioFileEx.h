...
public:
	...
	virtual BOOL			Open( LPCTSTR lpszFileName, UINT nOpenFlags, SFE_SAVEAS nSaveAs = SFE_ASCOMPILED, CFileException* pError = NULL );
	virtual BOOL			ReadString(CString& rString);
	virtual LPTSTR			ReadString(LPTSTR lpsz, UINT nMax);
	virtual void			WriteString( LPCTSTR lpsz );
	STDIOEXLONG				GetApproxCharCount();
	...
	bool					IsFileUnicode()	{ return m_bIsUnicodeText; }	
	static bool				IsFileUnicode(const CString& sFilePath);

	void					SetSaveAs(SFE_SAVEAS nSaveAs);
	void					SetCodePage(const UINT nCodePage);
	...
	static int		GetUnicodeStringFromMultiByteString(IN LPCSTR szMultiByteString, OUT wchar_t* szUnicodeString,
																			IN const size_t nUnicodeBufferSize, IN UINT nCodePage);
	...
	// Exceptions:	None.
	//
	static int		GetRequiredUnicodeLengthFromMultiByteString(IN LPCSTR szMultiByteString, IN UINT nCodePage);
	...
	// Exceptions:	None.
	//
	static int		GetNewUnicodeStringFromMultiByteString(IN LPCSTR szMultiByteString, IN OUT CTemplateSmartPtrArray<wchar_t>& spUnicodeString, IN const int nDefaultBufferSize,
																			 IN UINT nCodePage);
	...
	static int			GetMultiByteStringFromUnicodeString(IN const wchar_t * szUnicodeString, OUT char* szMultiByteString,
																			IN const int nMultiByteBufferSize,IN UINT nCodePage,
																			IN char cFillerChar);
	...
	//
	static int			GetRequiredMultiByteLengthForUnicodeString(IN const wchar_t * szUnicodeString,IN UINT nCodePage);
	...
	// Exceptions:	None.
	//
	static int			GetNewMultiByteStringFromUnicodeString(IN const wchar_t * szUnicodeString,IN OUT CTemplateSmartPtrArray<char>& spMultiByteString, IN const int nDefaultBufferSize,IN UINT nCodePage,
																			IN char cFillerChar);
...
protected:
	UINT	ProcessSaveFlags(UINT& nOpenFlags, SFE_SAVEAS nSaveAs);
	int    CheckReadBOM();
	BOOL	ReadUnicodeLine(OUT CString& sOutputLine);
	BOOL	ReadMultiByteLine(OUT CString& sOutputLine);
	UINT	WriteUnicodeLine(IN LPCTSTR sInputLine);
	UINT	WriteMultiByteLine(IN LPCTSTR sInputLine);
	static bool WantUnicode(SFE_SAVEAS nSaveAs);
