...
bool CFMindImporter::Import(LPCTSTR szSrcFilePath, ITaskList* pDestTaskFile, BOOL /*bSilent*/, IPreferences* /*pPrefs*/, LPCTSTR /*szKey*/)
{
	...
	return ImportTask(pFMTask, pTL10, NULL, TRUE); // NULL ==  root
}
...
bool CFMindImporter::ImportTask(const CXmlItem* pFMTask, ITaskList10* pDestTaskFile, HTASKITEM hParent, BOOL bAndSiblings)
{
	...
	// times
	TCHAR cUnits = (TCHAR)GetAttribValueI(pFMTask, FM_CUSTOMTIMEESTUNITS);
	pDestTaskFile->SetTaskTimeEstimate(hTask, GetAttribValueD(pFMTask, FM_CUSTOMTIMEEST), cUnits);

	cUnits = (TCHAR)GetAttribValueI(pFMTask, FM_CUSTOMTIMESPENTUNITS);
