...
bool CFMindExporter::Export(const ITaskList* pSrcTaskFile, LPCTSTR szDestFilePath, BOOL /*bSilent*/, IPreferences* /*pPrefs*/, LPCTSTR /*szKey*/)
{
	...
	return (FileMisc::SaveFile(szDestFilePath, sOutput, SFE_MULTIBYTE) != FALSE);
}
								...
								CXmlItem* pXIDestParent, int LEVEL, BOOL bAndSiblings)
{
	...
	// times
	TCHAR cUnits = 0;
	ADDCUSTOMATTRIB(FM_CUSTOMTIMEEST, pSrcTaskFile->GetTaskTimeEstimate(hTask, cUnits, FALSE))
	ADDCUSTOMATTRIB(FM_CUSTOMTIMEESTUNITS, cUnits)
	ADDCUSTOMATTRIB(FM_CUSTOMTIMESPENT, pSrcTaskFile->GetTaskTimeSpent(hTask, cUnits, FALSE))
	ADDCUSTOMATTRIB(FM_CUSTOMTIMESPENTUNITS, cUnits)
