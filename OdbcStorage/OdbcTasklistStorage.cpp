...
CString COdbcTasklistStorage::GetPreferenceKey(LPCTSTR szKey) const
{
	CString sKey(szKey);
...
{
	CString sKey = GetPreferenceKey(szKey);
	int nNumDb = pPrefs->GetProfileInt(sKey, _T("NumDatabases"));
...
void COdbcTasklistStorage::SaveDatabaseSetups(IPreferences* pPrefs, LPCTSTR szKey, const COdbcDatabaseSetupArray& aDbSetup) const
{
	...
	pPrefs->WriteProfileInt(sKey, _T("NumDatabases"), nNumDb);
