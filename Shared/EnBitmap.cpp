...
HBITMAP CEnBitmap::LoadImageFile(LPCTSTR szImagePath, COLORREF crBack, int cx, int cy)
{
	...
	{
	case FT_BMP:
		// The reason for loading BMP's explicitly instead of using IPicture
		// is that it is more efficient than creating device contexts etc that
		// the IPicture methods requires.
...
BOOL CEnBitmap::CopyImage(HICON hIcon, COLORREF crBack)
{
	ASSERT(m_hObject == NULL);      // only attach once, detach on destroy
	...
	if (SUCCEEDED(hr) && pPicture)
	{
		...
		if (cx && cy)
		{
			HBITMAP hbm = ExtractBitmap(pPicture, crBack, cx, cy);
			VERIFY(Attach(hbm));
		}
...
BOOL CEnBitmap::CopyImageFileToClipboard(HWND hWnd, LPCTSTR szImagePath, COLORREF crBack)
{
	...
	if (bm.LoadImage(szImagePath, crBack) && bm.CopyToClipboard(hWnd))
	{
