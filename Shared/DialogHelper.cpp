...
void CDialogHelper::ResizeButtonStaticTextFieldsToFit(CWnd* pParent)
{
	...
	{
		// prepare DC
		CDC* pDCRef = pParent->GetDC();
		CFont* pFont = CFont::FromHandle(GetFont(pParent));
		ASSERT(pFont);

		CFont* pOldFont = pDCRef->SelectObject(pFont);

		// resize children
		while (pChild)
		{
			ResizeButtonStaticTextToFit(pParent, pChild, pDCRef);
			pChild = pChild->GetWindow(GW_HWNDNEXT);
		}

		// restore DC
		pDCRef->SelectObject(pOldFont);
		pParent->ReleaseDC(pDCRef);
	}
}
...
void CDialogHelper::ResizeButtonStaticTextToFit(CWnd* pParent, CWnd* pCtrl, CDC* pDCRef)
{
	...
	if (!bCallerDC)
	{
		CDC* pDCRef = pCtrl->GetDC();
		CFont* pFont = CFont::FromHandle(GetFont(pCtrl));
		ASSERT(pFont);
		
		pOldFont = pDCRef->SelectObject(pFont);
	}
	ASSERT(pDCRef);
