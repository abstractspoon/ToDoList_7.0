										   ...
										   int nByStorage, IPreferences* pPrefs, BOOL bSilent)
{
	...
	if (nByStorage >= 0 && nByStorage < m_aStorage.GetSize())
	{
		...
		if (pStorage->RetrieveTasklist(pFInfo, pDestTaskFile, pPrefs, PREF_KEY, (bSilent != FALSE)))
		{
			// add storageID
										...
										int nByStorage, IPreferences* pPrefs, BOOL bSilent)
{
	...
	if (nByStorage >= 0 && nByStorage < m_aStorage.GetSize())
	{
		...
		if (pStorage->StoreTasklist(pFInfo, pSrcTaskFile, pPrefs, PREF_KEY, (bSilent != FALSE)))
		{
			pFInfo->sStorageID = pStorage->GetTypeID();
			return TRUE;
		}
