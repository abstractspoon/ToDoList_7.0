...
public:
	...
	BOOL Save(const CString& sFilePath, SFE_SAVEAS nSaveAs = SFE_ASCOMPILED);
	...
	// extended interface
	BOOL Open(const CString& sFilePath, XF_OPEN nOpenFlags, SFE_SAVEAS nSaveAs = SFE_ASCOMPILED);
	virtual BOOL SaveEx();
	virtual BOOL LoadEx(const CString& sRootItemName = EMPTY_STR, IXmlParse* pCallback = NULL);
	void Close();
	int GetLastFileError() const;

	BOOL IsFileUnicode() { return CStdioFileEx::IsFileUnicode(); }	
	void SetSaveAs(SFE_SAVEAS nSaveAs) { CStdioFileEx::SetSaveAs(nSaveAs); }
