...
{
public:
	CTLSHoldResync(CTreeListSyncer& tls, HWND hwnd = NULL);
	~CTLSHoldResync();
...
protected:
	...
	static BOOL PointInRect(POINT point, HWND hWnd, BOOL bScreenCoords);
	static BOOL OsIsXP();
	...
	BOOL ResyncScrollPos(HWND hwnd, HWND hwndTo);
	BOOL ResyncSelection(HWND hwnd, HWND hwndTo, BOOL bClearTreeSel = TRUE);
