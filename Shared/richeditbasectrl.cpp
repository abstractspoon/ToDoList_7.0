...
void CRichEditBaseCtrl::EnableAutoSpellChecking()
{
	ASSERT(GetSafeHwnd());

	DWORD dwLangOpt = SendMessage(EM_GETLANGOPTIONS);
	SendMessage(EM_SETLANGOPTIONS, 0, (dwLangOpt | IMF_SPELLCHECKING));
	
	DWORD dwEditStyle = SendMessage(EM_GETEDITSTYLE);
	SendMessage(EM_SETEDITSTYLE, 0, (dwEditStyle | SES_USECTF | SES_CTFALLOWEMBED | SES_CTFALLOWSMARTTAG | SES_CTFALLOWPROOFING));
}
