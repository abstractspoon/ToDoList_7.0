...
BOOL FileMisc::IsFolderWritable(LPCTSTR szFolder, BOOL bAutoCreate)
{
	if (bAutoCreate)
		CreateFolder(szFolder);
...
{
	if (IsLoggingEnabled())
		VERIFY(AppendLineToFile(GetLogFilePath(), szText));
}
...
void FileMisc::LogText(LPCTSTR szText, ...)
{
	...
	VERIFY(AppendLineToFile(GetLogFilePath(), sLogLine));
}

BOOL FileMisc::AppendLineToFile(LPCTSTR szPathname, const CString& sLine, SFE_SAVEAS nSaveAs)
{
	if (sLine.IsEmpty())
		return FALSE;
...
BOOL FileMisc::SaveFile(LPCTSTR szPathname, const CString& sText, SFE_SAVEAS nSaveAs, UINT nMultibyteCodePage)
{
	CStdioFileEx file;
...
{
	return GetModuleVersion(NULL, aVersionParts);
}
...
BOOL FileMisc::IsPathInSpecialFolder(const CString& sFilePath, int nCSIDL)
{
	...
	return (sFilePath.Left(sSpecialFolder.GetLength()).CompareNoCase(sSpecialFolder) == 0);
}

BOOL FileMisc::IsAppInProgramFiles()
{
	return IsPathInSpecialFolder(GetAppFilePath(), CSIDL_PROGRAM_FILES);
}
