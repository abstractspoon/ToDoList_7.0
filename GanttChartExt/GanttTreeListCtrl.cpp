...
BOOL CGanttTreeListCtrl::GetSelectedTaskDates(COleDateTime& dtStart, COleDateTime& dtDue) const
{
	...
	if (GetStartDueDates(*pGI, dtStart, dtDue))
	{
		...
		COleDateTime dtDuration(dtDue.m_dt - dtStart.m_dt);

		if (CDateHelper::GetTimeOnly(dtDuration).m_dt >= END_OF_DAY)
		{
...
void CGanttTreeListCtrl::UpdateTasks(const ITaskList* pTasks, IUI_UPDATETYPE nUpdate, int nEditAttribute)
{
	...
	case IUI_EDIT:
		{
			...
			// update the task(s)
			if (UpdateTask(pTasks14, pTasks14->GetFirstTask(), nUpdate, nEditAttribute, TRUE))
			{
				...
				nRecalcCol = nEditAttribute;
				bResort = (m_nSortBy != GTLCC_NONE) && (m_nSortBy == MapAttribToColumn(nEditAttribute));
			}
		}
		break;
	...
	case IUI_DELETE:
		{
			...
			RemoveDeletedTasks(NULL, pTasks14);
	...
	InitItemHeights();
	RecalcTreeColumns(nRecalcCol, TRUE);
...
GTLC_COLUMN CGanttTreeListCtrl::MapAttribToColumn(int nEditAttrib)
{
	switch (nEditAttrib)
	{
	case TDCA_TASKNAME:		return GTLCC_TITLE;
	case TDCA_DONEDATE:		return GTLCC_ENDDATE;
	case TDCA_DUEDATE:		return GTLCC_ENDDATE;
...
void CGanttTreeListCtrl::RemoveDeletedTasks(HTREEITEM hti, const ITaskList14* pTasks)
{
...
void CGanttTreeListCtrl::OnHeaderDividerDblClk(NMHEADER* pHDN)
{
	...
	{
		RecalcTreeColumnWidth((GTLC_COLUMN)nCol);
		Resize();
	}
...
void CGanttTreeListCtrl::SetDoneTaskAttributes(COLORREF color, BOOL bStrikeThru)
{
	...
	if (bStrikeThru)
	{
		HFONT hFont = (HFONT)::SendMessage(m_hwndTree, WM_GETFONT, 0, 0);
...
void CGanttTreeListCtrl::DrawTreeItem(CDC* pDC, HTREEITEM hti, int nCol, const GANTTITEM& gi, BOOL bSelected, COLORREF crBack)
{
	...
	if (!sItem.IsEmpty())
	{
		...
		int nAlign = DT_LEFT;
		BOOL bLighter = FALSE; 
		UINT nFlags = (DT_VCENTER | GraphicsMisc::GetRTLDrawTextFlags(m_hwndTree));
		...
		HFONT hFontOld = (gi.IsDone() ? (HFONT)pDC->SelectObject(m_hFontDone) : NULL);
...
COLORREF CGanttTreeListCtrl::GetTreeTextBkColor(const GANTTITEM& gi, BOOL bSelected, BOOL bAlternate) const
{
	...
	else if (HasOption(GTLCF_TASKTEXTCOLORISBKGND) && gi.HasTextColor() && !gi.IsDone())
	{
		return gi.crText;
	}
	else if (bAlternate && HasAltLineColor())
	{
		return m_crAltLine;
	}
...
{
	COLORREF crText = gi.crText; // default

	if (gi.IsDone() && (m_crDone != CLR_NONE))
	{
		crText = m_crDone;
	...
	else if (gi.HasTextColor())
	{
		if (!bSelected && HasOption(GTLCF_TASKTEXTCOLORISBKGND) && !gi.IsDone())
		{
			crText = GraphicsMisc::GetBestTextColor(gi.crText);
		}
		else
		{
			crText = gi.crText;
...
BOOL CGanttTreeListCtrl::CalcDependencyEndPos(int nItem, GANTTDEPENDENCY& depend, BOOL bFrom, LPPOINT lpp) const
{
	...
	// Assume the end is not 'in shot' and pick
	// whichever is the most appropriate end
	if (bFrom && !pGD->IsStartSet())
	{
		nPos = 0;
		nScrollPos = 0;
	}
	else if (!bFrom && !pGD->IsEndSet())
	{
		CRect rClient;
		::GetClientRect(m_hwndList, rClient);
...
void CGanttTreeListCtrl::DeleteTreeItem(HTREEITEM hti)
{
	...
	if (nItem != -1)
		ListView_DeleteItem(m_hwndList, nItem);
...
{
	// tree columns
	CDC* pDC = CDC::FromHandle(::GetDC(m_hwndTree));
	CFont* pOldFont = GraphicsMisc::PrepareDCFont(pDC, m_hwndTree);
	
	int nCol = m_treeHeader.GetItemCount();

	while (nCol--)
		RecalcTreeColumnWidth((GTLC_COLUMN)nCol, pDC);
	
	pDC->SelectObject(pOldFont);
	::ReleaseDC(m_treeHeader, *pDC);
...
BOOL CGanttTreeListCtrl::RecalcTreeColumns(int nEditAttribute, BOOL bResize)
{
	BOOL bTitle = (WantAttributeUpdate(nEditAttribute, GTLCC_TITLE) && !m_treeHeader.IsItemTracked(GTLCC_TITLE));
	BOOL bAllocTo = (WantAttributeUpdate(nEditAttribute, GTLCC_ALLOCTO) && !m_treeHeader.IsItemTracked(GTLCC_ALLOCTO));
	...
	if (bTitle || bAllocTo)
	{
		...
		if (bTitle)
			RecalcTreeColumnWidth(GTLCC_TITLE, pDC);
			
		if (bAllocTo)
			RecalcTreeColumnWidth(GTLCC_ALLOCTO, pDC);
...
void CGanttTreeListCtrl::RecalcTreeColumnWidth(int nCol, CDC* pDC)
{
	...
	if (bNullDC)
	{
		pDC = CDC::FromHandle(::GetDC(m_hwndTree));
		pOldFont = GraphicsMisc::PrepareDCFont(pDC, m_hwndTree);
	}
	...
	{
	case GTLCC_TITLE:
		TCH()->FindWidestItem(nColWidth, TRUE, pDC);
	...
	{
		pDC->SelectObject(pOldFont);
		::ReleaseDC(m_treeHeader, pDC->Detach());
	}
}
...
DWORD CGanttTreeListCtrl::ListHitTestTask(const CPoint& point, BOOL bScreen, GTLC_HITTEST& nHit) const
{
	...
	double dDateTol = CalcDateDragTolerance(nCol);
		...
		if (GetValidDragDate(ptCursor, dtDrag))
		{
			...
			if (m_bDragging) 
			{
				...
				if (CDateHelper::DateHasTime(dtStart) && 
					...
					!CDateHelper::DateHasTime(dDuration))
				{
					dDuration -= 1.0;
					dDuration += END_OF_DAY;
				}
